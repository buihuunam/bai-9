...
// Hàm tính điểm trung bình (loại bỏ điểm thấp nhất)
...
int main() {
    ...
    while (soLuongDiem <= 1) {
        cout << "Số lượng điểm không hợp lệ. Vui lòng nhập lại (ít nhất 2 điểm): ";
        cin >> soLuongDiem;
    }
    ...
    // Tính điểm trung bình (loại bỏ điểm thấp nhất)
...
double tinhDiemTrungBinh(int* diem, int soLuong) {
    ...
    // Bỏ đi điểm thấp nhất (ở vị trí đầu tiên sau khi sắp xếp)
    for (int i = 1; i < soLuong; ++i) {
        tongDiem += diem[i];
    }

    // Ép kiểu sang double để tính trung bình
    return static_cast<double>(tongDiem) / (soLuong - 1);
}
...
void hienThiKetQua(int* diem, int soLuong, double diemTrungBinh) {
    ...
    cout << "\nĐiểm trung bình (loại bỏ điểm thấp nhất): " << diemTrungBinh << endl;
}
